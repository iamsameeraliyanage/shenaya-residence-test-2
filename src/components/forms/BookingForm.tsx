...
export default function BookingForm({ formFields, pageContent, rooms, roomLabels }: BookingFormProps) {
  ...
  const onSubmit: SubmitHandler<BookingFormData> = async (data) => {
    try {
      ...
      console.log('Reservation form submitted:', { ...data, selectedRooms, numberOfGuests });
      ...
      const response = await fetch(`${apiUrl}/book`, {
        ...
        body: JSON.stringify({
          ...
          roomType: selectedRooms.length > 0 ? rooms.filter(room => 
            selectedRooms.includes(room.id)
          ).map(room => room.name).join(', ') : 'No rooms selected',
      ...
      if (response.ok) {
        ...
        setSelectedRooms([]);
        setNumberOfGuests('');
        alert('Thank you! Your reservation request has been submitted successfully. You will receive a confirmation email shortly.');
      } else {
  ...
  return (
    <div className="max-w-4xl mx-auto bg-white rounded-lg shadow-lg overflow-hidden">
      <form onSubmit={handleSubmit(onSubmit)} className="p-8">
        ...
        <div className="mb-8">
          <h2 className="text-2xl font-semibold mb-6" style={{ color: '#B09244' }}>
                ...
                {field.type === 'select' ? (
                  <div>
                    <select
                      ...
                      className={`w-full px-4 py-3 border rounded-md focus:outline-none focus:ring-2 focus:ring-yellow-600 focus:border-transparent text-gray-900 ${
                        errors[field.name as keyof BookingFormData] ? 'border-red-500' : 'border-gray-300'
                      }`}
                    >
                  ...
                  <div>
                    <input
                      ...
                      {...register(field.name as keyof BookingFormData, {
                        required: field.required ? `${field.label} is required` : false,
                        ...(field.type === 'date' && field.name === 'checkInDate' && {
                          min: getTomorrowDate(),
                        ...
                        ...(field.type === 'date' && field.name === 'checkOutDate' && {
                          min: getMinCheckOutDate(),
                          validate: (value: string | string[] | undefined) => {
                            if (!value || Array.isArray(value)) return true;
                            const checkInValue = watch('checkInDate');
                            if (!checkInValue || Array.isArray(checkInValue)) return true;
                            const checkInDate = new Date(checkInValue);
                            const checkOutDate = new Date(value);
                            return checkOutDate > checkInDate || 'Check-out date must be after check-in date';
                          }
                        }),
                        ...(field.name === 'numberOfGuests' && field.type === 'select' && {
                          min: '1'
                        })
                      })}
                      placeholder={field.placeholder}
                      className={`w-full px-4 py-3 border rounded-md focus:outline-none focus:ring-2 focus:ring-yellow-600 focus:border-transparent text-gray-900 placeholder-gray-600 ${
                        errors[field.name as keyof BookingFormData] ? 'border-red-500' : 'border-gray-300'
          ...
          {formFields.slice(6).map((field) => (
            <div key={field.id} className="mb-6">
              ...
              <textarea
                ...
                placeholder={field.placeholder}
                rows={field.rows}
                className={`w-full px-4 py-3 border rounded-md focus:outline-none focus:ring-2 focus:ring-yellow-600 focus:border-transparent resize-none text-gray-900 placeholder-gray-600 ${
                  errors[field.name as keyof BookingFormData] ? 'border-red-500' : 'border-gray-300'
                }`}
              />
              {errors[field.name as keyof BookingFormData] && (
